...
def load_mask(image_path):
    ...
    #cv2.imwrite(sub_output_path+"/Binary Image.png", image)
    #cv2.waitKey()
    ...
    #mask = np.array(image) > 128  # Convert to boolean mask (True for white, False for black)
    mask=binary_image
    return mask


binmask = load_mask(BINSHAPE_BLACKANDWHITE)  # Load mask from image

BINWIDTH =binmask.shape[1] 
