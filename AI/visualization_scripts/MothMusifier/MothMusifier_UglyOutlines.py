...
while running:
    ...
    for i, (x, y) in enumerate(circle_positions):
        ...
        outline_growth[i] += 2  # Increment outline growth twice as fast

        # Draw expanding filled outline
        outline_color = (
        ...
        expanded_points = [
            (
                int(x + (point[0] - mask.centroid()[0]) * (1 + outline_growth[i] * 0.02)),
                int(y + (point[1] - mask.centroid()[1]) * (1 + outline_growth[i] * 0.02))
            )
