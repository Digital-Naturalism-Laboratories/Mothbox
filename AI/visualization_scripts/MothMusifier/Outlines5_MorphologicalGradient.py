...
def overlay_color_with_mask(background_img, mask_img, color):
  ...
  #mask_rgb = cv2.medianBlur(mask_rgb.astype(np.uint8) ,ksize=15)
  #mask_rgb = cv2.morphologyEx(mask_rgb, cv2.MORPH_GRADIENT,(35,35)) 
  #mask_rgb= cv2.dilate(mask_rgb, (25,25), iterations=20)
  #mask_rgb= cv2.erode(mask_rgb, (15,15), iterations=2)
...
def median_blur_color(image, ksize):
  """
  Applies median blur to a color image while preserving color channels.

  Args:
    image: The input image as a NumPy array (e.g., read using cv2.imread()).
    ksize: The size of the kernel for median blurring (must be an odd integer).
  ...
  # Apply median blur to each channel separately
  b_blur = cv2.medianBlur(b.astype(np.uint8), ksize)
  g_blur = cv2.medianBlur(g.astype(np.uint8), ksize)
  r_blur = cv2.medianBlur(r.astype(np.uint8), ksize)
...
def animate_alpha_overlay(image_path, max_iterations=40, delay=1):
    """
    ...
    try:
        ...
        while True:
            ...
            if i % 2 == 0:
                ...
                dilated_alpha = cv2.erode(dilated_alpha, shrinkkernel, iterations=1)
                #pastel_color=(0,0,0)

                None
                dilated_alpha = cv2.morphologyEx(dilated_alpha, cv2.MORPH_GRADIENT,kernel) 



            #dilated_alpha = cv2.dilate(dilated_alpha, kernel, iterations=2)
            #dilated_alpha = cv2.blur(dilated_alpha,(50,50))

            #dilated_alpha = cv2.filter2D(dilated_alpha,-1, shrinkkernel)
            #dilated_alpha = cv2.medianBlur(dilated_alpha.astype(np.uint8) ,ksize=15)
            #dilated_alpha=median_blur_color(dilated_alpha, 9) 
            #dilated_alpha = cv2.erode(dilated_alpha, kernel, iterations=1)

            #dilated_alpha = cv2.erode(dilated_alpha, kernel, iterations=1)

            #dilated_alpha = cv2.morphologyEx(dilated_alpha, cv2.MORPH_GRADIENT,kernel) 
            #dilated_alpha=cv2.Sobel(dilated_alpha,cv2.CV_16S,1,1,ksize=5,scale=1,delta=0)
...
"""
            ...
            if i % 2 == 0:
                dilated_alpha=overlay_color_with_mask(dilated_alpha,thresholded_alpha,(0,0,0))
            else:
                dilated_alpha=overlay_color_with_mask(dilated_alpha,cv2.dilate(thresholded_alpha, kernel, iterations=2), pastel_color )
