...
outline_growth = [1] * len(circle_positions)  # Initialize outline growth rates
...
while running:
    ...
    for i, (x, y) in enumerate(circle_positions):
        ...
        outline_points = mask.outline()  # Get outline points
        expanded_points = [
            (
                int(x + (point[0] - mask.centroid()[0]) * (1 + outline_growth[i] * 0.01)),
                int(y + (point[1] - mask.centroid()[1]) * (1 + outline_growth[i] * 0.01))
            )
            for point in outline_points
        ]
        if(len(expanded_points)>2):
            pygame.draw.polygon(screen, outline_color, expanded_points, width=2)
