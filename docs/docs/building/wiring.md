...
Finally, we will connect all the electronics to make our Mothbox work! There is no soldering required, just lever nut connectors and wire strippers.
We worked hard to keep the electronics as simple as possible so many people will be able to build this. There may be more "elegant" ways of connecting everything together, but we tried to make the wiring style as easy to understand as possible. 
...
You need a plug and a socket DC barrel jack with cables. And you can also use a inline lever nut to connect them.
...
Mark your spot for the port. You can use a marker to note the spot. Right between the big gap between the flanges is a good spot. You might notice my pen mark is a bit to the left. That works, but actually around where I put the red dot should be easier.
...
If you are concerned about moisture, you can hot glue or silicone around your port to make it extra secure.
...
Sometimes you want to temporarily disable the mothbox from running. For instance, if you are travelling overnight somewhere, you might not want 20 mothboxes to start lighting up in the backseat of your car. There is a simple way to add two wires that let you ARM and DISARM the Mothbox.
...
Whenever those two pins are DISCONNECTED from each other, the Mothbox is ARMED and ready to go!
