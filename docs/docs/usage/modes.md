...
(Hourly and Daily SwitchesO - For setting the schedule in "Switches" mode

This is organized into a clear, professional markdown table format that uses appropriate headings and formatting for easy reading.

***

### Device Controls & Modes Reference

| Button ID | Primary Function | Description |
| :--- | :--- | :--- |
| **U1** | **Switch Mode Toggle** | When enabled, the scheduling system overrides the internal schedule defined in `mothbox_settings.csv` and uses the sequence determined by the physical switches. |
| **C1** | **Diagnostic / Party Mode** | Toggles a special diagnostic or "Party" mode. |
| **A1, A2, A3, EXT** | **Free Switches** | Customizable outputs. These are independent toggles in the firmware not currently connected to any fixed function and can be assigned custom actions via programming. |
| **HI** | **Hi Power Mode Placeholder** | Reserved for the upcoming "Hi Power Mode" when the Mothbox is equipped with constant power and stable data connectivity. |
| **DEBUG** | **Debug Mode Activator** | Activates an extended "Debug" state, specifically keeping the Wi-Fi connection active and disabling all normal automated routines to allow for manual troubleshooting. |
...
## MOTHBOX DIY MODES
For the **Mothbox DIY**, your options are a bit more limited, but you can set what Mode your device is in by using Jumper wires on some select pins:
