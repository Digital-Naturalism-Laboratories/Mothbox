...
if __name__ == "__main__":
  ...
  if disks:
    ...
    for disk_name, capacity in sorted_disks:
      print(f"{disk_name}: total size {capacity[0]} GB - available size {capacity[1]} GB")
  else:
        ...
        try:
            ...
            if desktop_available < x * 1024**3:  # x GB in bytes
              ...
              print("Original photos deleted after being backed up due to low internal storage.")
                ...
                print("More than "+str(x)+ "GB remain so original files are also kept in internal storage after backing up to external storage")
            print("we have finished backing up! yay!")
            break
        except subprocess.CalledProcessError as err:
            print("Error during backup, moving to next available storage if there is one!:", err)
            thingsworkedok=False
      else:
