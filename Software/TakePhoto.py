...
# Note andy is probably using global variables in python incorrectly
global onlyflash
onlyflash = False
        ...
        with open(file_path) as csv_file:
            ...
            for row in reader:
                ...
                if setting == "LensPosition":
                    ...
                    value = (
                        value.lower() == "true"
                    )  # Convert to bool (adjust logic if needed)
                elif (
                    setting == "AwbMode"
...
def print_af_state(request):
    ...
    print(
        ("Idle", "Scanning", "Success", "Fail")[md["AfState"]], md.get("LensPosition")
    )


def run_calibration():
    preview_config = picam2.create_preview_configuration(
        main={"format": "RGB888", "size": (1920 * 2, 1080 * 2)}
    )
    ...
    new_settings = {
        ...
        "ExposureTime": calib_exposure + exposure_shift,
    }
...
def list_exposuretimes(middle_exposuretime, num_photos, exposure_width):
    ...
    half_num_photos = int(
        (num_photos - 1) / 2
    )  # Ensure at least one photo on each side
...
def takePhoto_Manual():
    ...
    exposure_times = list_exposuretimes(
        middleexposure, HDR_NUM_PHOTOS, HDR_EXPOSURETIME_WIDTH
    )
    ...
    exposureset_delay = (
        0.3  # values less than 5 don't seem to work! (unless you restart the cam!)
    )
    requests = []  # Create an empty list to store requests
    PILs = []
    metadatas = []
    # HDR loop
    for i in range(HDR_NUM_PHOTOS):
        ...
        time.sleep(
            exposureset_delay
        )  # need some time for the settings to sink into the camera)
        flashOn()
    ...
    for img in PILs:
        ...
        if ImageFileType == 1:  # png
            filepath = (
                folderPath + COMPUTER_NAME + "_" + timestamp + "_HDR" + str(i) + ".png"
            )
        elif ImageFileType == 0:  # jpeg
            filepath = (
                folderPath + COMPUTER_NAME + "_" + timestamp + "_HDR" + str(i) + ".jpg"
            )
        elif ImageFileType == 2:  # bmp
            filepath = (
                folderPath + COMPUTER_NAME + "_" + timestamp + "_HDR" + str(i) + ".bmp"
            )
        ...
        exif_ifd = {  # piexif.ExifIFD.DateTimeOriginal: u"2099:09:29 10:10:10",
            piexif.ExifIFD.ExposureTime: (
                1,
                int(1 / (abs(exposure_times[i]) / 1000000)),
            ),
            ...
            piexif.ExifIFD.ISOSpeedRatings: int(
                camera_settings.get("AnalogueGain") * 100
            ),
        }
        gps_ifd = {}
        first_ifd = {
...
# HDR settings
HDR_NUM_PHOTOS = int(
    camera_settings.pop("HDR", HDR_NUM_PHOTOS)
)  # defaults to what is set above if not in the files being read
HDR_EXPOSURETIME_WIDTH = int(camera_settings.pop("HDR_width", HDR_EXPOSURETIME_WIDTH))
if HDR_NUM_PHOTOS < 1 or HDR_NUM_PHOTOS == 2:
    HDR_NUM_PHOTOS = 1
...
capture_config = picam2.create_still_configuration(
    main=capture_main, raw=None, lores=None
)
